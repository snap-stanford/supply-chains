...
def get_args():
    ...
    parser.add_argument('--tesla_filepath', help='Path to the .csv file that contains the Tesla supply-chain transactions', default = None)
...
def preprocess_tesla(df, start_date = "2019-01-01", use_titles = True, id2company = None,
    ...
    df_tesla_agg = df_tesla_agg.rename(columns = {"quantity": "total_quantity",
                                                 "weight": "total_weight",
                                                 "amount": "total_amount",
                                                 "bill": "bill_count"})
...
if __name__ == "__main__":
    ...
    args = get_args()
    df = pd.read_csv(args.tesla_filepath)
    print("Loaded in {} raw entries from the Tesla dataset at {}!\n...".format(len(df), args.tesla_filepath))
