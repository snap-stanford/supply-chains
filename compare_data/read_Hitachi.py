...
def parse_date(date_str: str) -> tuple[int]:
    """
    helper function: takes a string of the form YY-MM-DD
...
def get_hitachi_products(csv_file = "./data/Hitachi/hs_category_description.csv", hs_level = 6):
    """
    mapping from Hitachi's hs products to descriptions,
    either at the 2, 4, and 6-digit (default) levels, specified by hs_level
    """
    ...
    for i in range(n):
        ...
        else: #using just 2 digits for product nomenclature
            hs2 = hs6 // 10**4
...
def get_hitachi_countries_util(csv_file = "./data/Hitachi/country_region.csv"):
    """
    helper function: mapping from country ids in the Hitachi dataset 
    to the corresponding name, and globalised ISO codes
    """
    df = pd.read_csv(csv_file)
    n = len(df)
...
def get_hitachi_companies_util(csv_file = "./data/Hitachi/group_subsidiary_site.csv"):
    """
    helper function: mapping from company site ids in the Hitachi dataset 
    to the corresponding country ids they are located in
    """
    df = pd.read_csv(csv_file)
    n = len(df)
    code2company = {}
...
def get_hitachi_countries(country_csv = "./data/Hitachi/country_region.csv", company_csv = "./data/Hitachi/group_subsidiary_site.csv"):
    """
    composing the two functions above to directly map 
    from company site ids to the country name & ISO codes
    """
...
def get_transaction_years(start_date: str, end_date: str) -> dict:
    """
    given a start and end date (YY-MM-DD), return a dictionary of the number of days
    between those two dates (value), grouped by common year (key). 
    These days counts are normalized to a sum of 1. 
    """
