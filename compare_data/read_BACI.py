...
def get_entity_constructor(aggregation_type, country_map):
    """
    given an aggregation type (e.g. exporter_importer, importer) and map from country IDs to 
    global ISO codes, this returns a generator function to obtain the keys used for dataset 
    aggregation (each key corresponds to a distinct entity within the aggregation scheme, 
    such as USA or MEX under importer or tuple<USA, 100000> under importer_product)
    
    note: wanted to implement this with prettier code, but use of separate lambda functions
    optimises performance speed noticeably 
    """
...
if __name__ == "__main__":
    """
    for testing out the functions above and printing a results snippet
    """
