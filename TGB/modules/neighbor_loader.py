...
class LastNeighborLoaderTGNPL:
    ...
    def insert(self, src: Tensor, dst: Tensor, prod: Tensor):
        ...
        neighbors = torch.cat([src, prod, dst, prod], dim=0)
        nodes = torch.cat([prod, src, prod, dst], dim=0)
        e_id = torch.arange(
            self.cur_e_id, self.cur_e_id + src.size(0) + dst.size(0), device=src.device
        ).repeat(2)
        self.cur_e_id += src.numel() + dst.numel()
